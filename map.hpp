		...
		pair<iterator, iterator>		equal_range(const key_type& k)
		{
			...
			while (iter !=  end())
			{
				if (k_compare(iter->first, k) == false && k_compare(k, iter->first) == false)
					return (make_pair(lower_bound(iter->fiirst), upper_bound(iter->first)));
			}
